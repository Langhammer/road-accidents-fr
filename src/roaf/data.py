...
def df_testing_info(df):
    ...
    df_dtypes_and_na_counts = pd.DataFrame(
        {"dtypes": df.dtypes, "n_na": df.isna().sum()}
    )
...
def df_compare_to_description(df, description_filepath):
    ...
    pd.testing.assert_frame_equal(
        left=(pd.read_pickle(description_filepath)),
...
def read_csv_of_year(years=None, data_categories=None):
    ...
    for year in years:
        ...
        for this_category, this_sep in zip(data_categories, separators):
            ...
            this_file_path_and_name = (
                "../data/raw/annual_accidents/"
                + this_year_str
                + "/"
            ...
            this_df_dict[this_category] = pd.read_csv(
                this_file_path_and_name,
                encoding="utf-8",
                sep=this_sep,
                low_memory=False,
    ...
    for this_category in data_categories:
        dict_of_category_dfs[this_category] = pd.concat(
            [df_dict[year][this_category] for year in years], ignore_index=True
        )
...
def df_geotransform(df, lat_col="latitude", lon_col="longitude"):
    ...
    lat, lon = geotransformer.transform( # pylint: disable=E0633
